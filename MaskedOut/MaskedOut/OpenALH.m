...
+ (void)loadSoundNamed:(NSString *)name withFileName:(NSURL *)filePath withPitch:(float) pitch {
    ...
    alSourcef(outputSource, AL_PITCH, pitch);
...
+ (void)pauseSoundNamed:(NSString *)name {
    ...
    [self checkOpenAlError:@"pause source"];
}
...
+ (void)stopSoundNamed:(NSString *)name {
    ...
    [self checkOpenAlError:@"stop source"];
}

+ (bool)isPlaying {
    ALint sourceState;
    alGetSourcei(outputSource, AL_SOURCE_STATE, &sourceState);
